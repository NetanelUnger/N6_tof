...
/* USER CODE BEGIN USBPD_USBIF_Init */
  ...
  Debug_UART_Log("USBPD", "USB data interface initialized");
  return 0;
...
/* USER CODE BEGIN USBPD_USBIF_DeviceStart */
  ...
  Debug_UART_Log("USBPD", "Device START requested on port %lu", (unsigned long)PortNum);

  if (App_USBX_Device_RequestStart() != TX_SUCCESS)
  {
    Debug_UART_Log("USBPD", "ERROR: failed to queue device START");
    Error_Handler();
  }
  Debug_UART_Log("USBPD", "Device START queued");
...
/* USER CODE BEGIN USBPD_USBIF_DeviceStop */
  ...
  Debug_UART_Log("USBPD", "Device STOP requested on port %lu", (unsigned long)PortNum);

  if (App_USBX_Device_RequestStop() != TX_SUCCESS)
  {
    Debug_UART_Log("USBPD", "ERROR: failed to queue device STOP");
    Error_Handler();
  }
  Debug_UART_Log("USBPD", "Device STOP queued");
