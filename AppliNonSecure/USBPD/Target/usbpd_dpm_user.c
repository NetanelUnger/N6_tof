...
Debug_UART_Log("USBPD", "CAD cable event: port=%u state=%u",
               (unsigned int)PortNum, (unsigned int)State);
...
/* USER CODE BEGIN USBPD_DPM_Notification */
  Debug_UART_Log("USBPD", "Notification: port=%u event=%u data-role=%u",
                 (unsigned int)PortNum, (unsigned int)EventVal,
                 (unsigned int)DPM_Params[PortNum].PE_DataRole);
