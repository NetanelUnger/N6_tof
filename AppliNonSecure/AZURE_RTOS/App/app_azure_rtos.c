...
VOID tx_application_define(VOID *first_unused_memory)
{
  ...
  Debug_UART_Log("RTOS", "tx_application_define entered");
  ...
  if (tx_byte_pool_create(&tx_app_byte_pool, "Tx App memory pool", tx_byte_pool_buffer, TX_APP_MEM_POOL_SIZE) != TX_SUCCESS)
  {
    ...
    Debug_UART_Log("RTOS", "ERROR: application byte-pool creation failed");
  ...
  else
  {
    ...
    Debug_UART_Log("RTOS", "Application byte pool created");
    ...
    if (status != TX_SUCCESS)
    {
      ...
      Debug_UART_Log("RTOS", "ERROR: App_ThreadX_Init status=%u", (unsigned int)status);
      while(1)
      {
      }
    ...
    Debug_UART_Log("RTOS", "Application threads created");

    /* USER CODE END  App_ThreadX_Init_Success */

  }

  if (tx_byte_pool_create(&ux_device_app_byte_pool, "Ux App memory pool", ux_byte_pool_buffer, UX_APP_MEM_POOL_SIZE) != TX_SUCCESS)
  {
    /* USER CODE BEGIN UX_Byte_Pool_Error */
    Debug_UART_Log("USBX", "ERROR: USBX byte-pool creation failed");
  ...
  else
  {
    ...
    Debug_UART_Log("USBX", "USBX byte pool created");
    ...
    if (status != UX_SUCCESS)
    {
      ...
      Debug_UART_Log("USBX", "ERROR: MX_USBX_Init status=%u", (unsigned int)status);
      while(1)
      {
      }
    ...
    Debug_UART_Log("USBX", "USBX system and device thread initialized");
  ...
  if (tx_byte_pool_create(&usbpd_app_byte_pool, "USBPD App memory pool", usbpd_byte_pool_buffer, USBPD_DEVICE_APP_MEM_POOL_SIZE) != TX_SUCCESS)
  {
    ...
    Debug_UART_Log("USBPD", "ERROR: USBPD byte-pool creation failed");
  ...
  else
  {
    ...
    Debug_UART_Log("USBPD", "USBPD byte pool created");
    ...
    if (status != USBPD_OK)
    {
      ...
      Debug_UART_Log("USBPD", "ERROR: MX_USBPD_Init status=%u", (unsigned int)status);
      while(1)
      {
      }
    ...
    Debug_UART_Log("USBPD", "USBPD threads initialized; waiting for CN8 attach");
