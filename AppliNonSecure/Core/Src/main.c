...
int main(void)
{
  ...
  USBPD_PreInitOs();
  /* USER CODE BEGIN 2 */
  Debug_UART_Log("BOOT", "Peripheral initialization complete; starting ThreadX");
...
static void MX_GPDMA1_Init(void)
{
  ...
  Debug_UART_Log("BOOT", "GPDMA1 initialized");
...
static void MX_I3C1_Init(void)
{
  ...
  */
  ...
  Debug_UART_Log("BOOT", "I3C1 initialized");
...
static void MX_SPI4_Init(void)
{
  ...
  Debug_UART_Log("BOOT", "SPI4 initialized for GC9A01 display TX DMA");
...
static void MX_SPI5_Init(void)
{
  ...
  Debug_UART_Log("BOOT", "SPI5 initialized for ST67W6X radio");
...
static void MX_UCPD1_Init(void)
{
  ...
  Debug_UART_Log("BOOT", "UCPD1 initialized");
...
void MX_USB1_OTG_HS_PCD_Init(void)
{
  ...
  Debug_UART_Log("USBX", "Preparing USB1 OTG HS PCD handle");

  /* USER CODE END USB1_OTG_HS_Init 0 */

  /* USER CODE BEGIN USB1_OTG_HS_Init 1 */

  /* The handle may have been used by an earlier attach cycle.  Reset every
     field exactly as ST's N6 CDC ACM reference application does. */
  ...
  Debug_UART_Log("USBX", "HAL_PCD_Init returned; configuring endpoint FIFOs");

  /* The CDC data endpoint is High Speed with a 512-byte maximum packet.
     In non-DMA mode PCD_WriteEmptyTxFifo() waits until the FIFO can hold one
     complete maximum-size packet.  ST's reference value of 0x10 words for
     EP1 is only 64 bytes and therefore can never satisfy that condition.
     Give EP1 two full HS packets (0x100 words = 1024 bytes). */
  if ((HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_HS1, 0x200U) != HAL_OK) ||
      (HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS1, 0U, 0x10U) != HAL_OK) ||
      (HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS1, 1U, 0x100U) != HAL_OK) ||
      (HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS1, 2U, 0x20U) != HAL_OK))
  {
    Debug_UART_Log("USBX", "ERROR: USB endpoint FIFO configuration failed");
    Error_Handler();
  }

  Debug_UART_Log("USBX", "USB endpoint FIFOs configured");
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOE, CHIP_EN_Pin|LCD_RST_Pin|LCD_DC_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LCD_CS_Pin CHIP_EN_Pin */
  GPIO_InitStruct.Pin = LCD_CS_Pin|CHIP_EN_Pin;
  ...
  /*Configure GPIO pins : BOOT_Pin TOF_XSHUT_Pin */
  GPIO_InitStruct.Pin = BOOT_Pin|TOF_XSHUT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  ...
  Debug_UART_Log("BOOT", "GPIO initialized");
...
void Error_Handler(void)
{
  ...
  Debug_UART_Log("FATAL", "Error_Handler entered");
  __disable_irq();
  while (1)
  {
  }
