...
static int _get_async_frame_layout(uint16_t size, uint16_t *resolution,
                                   uint16_t *dss_size) {
    CHECK_NULL_PTR(resolution);
    CHECK_NULL_PTR(dss_size);

    if (size == RAW_BUFFER_SIZE(2)) {
        *resolution = FRAME_SIZE(2);
        *dss_size = DSS_SIZE(2);
    } else if (size == RAW_BUFFER_SIZE(4)) {
        *resolution = FRAME_SIZE(4);
        *dss_size = DSS_SIZE(4);
    } else if (size == RAW_BUFFER_SIZE(6)) {
        *resolution = FRAME_SIZE(6);
        *dss_size = DSS_SIZE(6);
    } else if (size == RAW_BUFFER_SIZE(8)) {
        *resolution = FRAME_SIZE(8);
        *dss_size = DSS_SIZE(8);
    } else if (size == RAW_BUFFER_SIZE(12)) {
        *resolution = FRAME_SIZE(12);
        *dss_size = DSS_SIZE(12);
    } else if (size == RAW_BUFFER_SIZE(24)) {
        *resolution = FRAME_SIZE(24);
        *dss_size = DSS_SIZE(24);
    } else {
        return VL53L9_ERROR_INVALID_PARAM;
    }
...
int vl53l9_frame_main_read_start_async(void *const p_dev,
                                       uint8_t *p_buffer, uint16_t size) {
    uint16_t resolution;
    uint16_t dss_size;
    int ret;
    ...
    CHECK_NULL_PTR(p_dev);
    CHECK_NULL_PTR(p_buffer);
    ret = _get_async_frame_layout(size, &resolution, &dss_size);
    CHECK_RET(ret);
    (void)dss_size;
    return vl53l9_read_async(p_dev, VL53L9_REGADDR_FB_DEPTH, p_buffer,
                             (uint32_t)resolution * 3U * 2U);
}

int vl53l9_frame_dss_map_start_async(void *const p_dev) {
    CHECK_NULL_PTR(p_dev);
    return vl53l9_write8_async(p_dev, VL53L9_REGADDR_COMMAND,
                               (uint8_t)COMMAND_DSS_LUT_MAP);
}

int vl53l9_frame_dss_read_start_async(void *const p_dev,
                                      uint8_t *p_buffer, uint16_t size) {
    uint16_t resolution;
    uint16_t dss_size;
    uint32_t offset;
    ...
    ret = _get_async_frame_layout(size, &resolution, &dss_size);
    CHECK_RET(ret);
    offset = (uint32_t)resolution * 3U * 2U;
    return vl53l9_read_async(p_dev, VL53L9_REGADDR_FB_DEPTH,
                             &p_buffer[offset], dss_size);
}

int vl53l9_frame_dss_unmap_start_async(void *const p_dev) {
    CHECK_NULL_PTR(p_dev);
    return vl53l9_write8_async(p_dev, VL53L9_REGADDR_COMMAND,
                               (uint8_t)COMMAND_DSS_LUT_UNMAP);
}

int vl53l9_frame_status_read_start_async(void *const p_dev,
                                         uint8_t *p_buffer, uint16_t size) {
    uint16_t resolution;
    uint16_t dss_size;
    uint32_t offset;
    ...
    ret = _get_async_frame_layout(size, &resolution, &dss_size);
    CHECK_RET(ret);
    offset = ((uint32_t)resolution * 3U * 2U) + dss_size;
    return vl53l9_read_async(p_dev, VL53L9_REGBASE_SENSOR_STATUS,
                             &p_buffer[offset], VL53L9_STATUS_SIZE);
}

int vl53l9_frame_ack_start_async(void *const p_dev) {
    CHECK_NULL_PTR(p_dev);
    return vl53l9_write8_async(p_dev, VL53L9_REGADDR_COMMAND,
                               (uint8_t)COMMAND_ACK_FRAME_READ);
}
...
int vl53l9_frame_command_status_start_async(void *const p_dev,
    ...
    return vl53l9_read_async(p_dev, VL53L9_REGADDR_COMMAND,
                             command_status, 1U);
}
