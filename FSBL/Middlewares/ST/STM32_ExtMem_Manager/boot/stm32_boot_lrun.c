...
BOOTStatus_TypeDef CopyApplication(void)
{
  ...
  switch (EXTMEM_GetMapAddress(EXTMEM_LRUN_SOURCE, &MapAddress))
  {
    case EXTMEM_OK :
    {
      ...
      (void)printf("[FSBL] NonSecure copied: bytes=0x%08lX, destination "
                   "vectors=0x%08lX/0x%08lX\r\n",
                   (unsigned long)img_size,
                   (unsigned long)*(const uint32_t *)(EXTMEM_LRUN_DESTINATION_ADDRESS_NS + EXTMEM_HEADER_OFFSET),
                   (unsigned long)*(const uint32_t *)(EXTMEM_LRUN_DESTINATION_ADDRESS_NS + EXTMEM_HEADER_OFFSET + 4U));
...
#if defined(EXTMEM_LRUN_TZ_ENABLE_NS)
  (void)printf("[FSBL] after D-cache shutdown: NonSecure vectors="
               "0x%08lX/0x%08lX\r\n",
               (unsigned long)*(const uint32_t *)(EXTMEM_LRUN_DESTINATION_ADDRESS_NS + EXTMEM_HEADER_OFFSET),
               (unsigned long)*(const uint32_t *)(EXTMEM_LRUN_DESTINATION_ADDRESS_NS + EXTMEM_HEADER_OFFSET + 4U));
#endif
