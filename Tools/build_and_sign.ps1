...
$SigningTool = $DevelopmentTools.SigningTool
...
$factoryArtifactNames = @(
    'N6_FSBL-trusted.bin',
    'N6_AppliSecure-trusted.bin',
    'N6_AppliNonSecure-trusted.bin',
    'N6-BootMetadata.bin'
)
$factoryHashes = [ordered]@{}
foreach ($artifactName in $factoryArtifactNames) {
    $factoryHashes[$artifactName] = Get-N6FileSha256 `
        -Path (Join-Path $OutputDir $artifactName)
}
$factoryManifest = [ordered]@{
    schema = 1
    firmware_version = $FirmwareVersion
    created_utc = [DateTime]::UtcNow.ToString('o')
    sha256 = $factoryHashes
}
[IO.File]::WriteAllText(
    (Join-Path $OutputDir 'factory-manifest.json'),
    (($factoryManifest | ConvertTo-Json -Depth 4) + [Environment]::NewLine),
    [Text.UTF8Encoding]::new($false))
Write-Host "Factory artifact manifest records firmware version $FirmwareVersion and all image hashes."
