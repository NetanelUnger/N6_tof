...
נשמרים. אין מחיקה אוטומטית של raw data, model או checkpoint.
...
- היא שולחת `MAP OFF`, לאחר מכן `DATASET STREAM ON`, ומציגה את מפת העומק.
...
- ה־BAT שואל בתחילתו כמה samples לשמור בכל מחלקה. ברירת המחדל היא 100;
  אפשר לבחור 40–60 לניסוי מהיר, אך validation יציג warning מתחת ל־100.
- פריימים נשמרים בקצב 3Hz כברירת מחדל. פריימים כמעט זהים מדולגים כדי לא למלא
  את הדאטה במאות העתקים של אותה תנוחה.
- כל לחיצה חדשה על רווח יוצרת `burst_id`. יש לצלם לפחות 3 bursts נפרדים לכל
  מחלקה, ועדיף sessions שונים, תאורה/רקע שונים ואנשים שונים. כברירת מחדל
  התוכנה מחשבת מגבלת burst שמבטיחה לפחות 3 קבוצות: המגבלה לעולם אינה עולה על
  25, וביעד 50 היא 17 (`17+17+16`). העצירה אינה חוסמת ומכריחה שינוי תנוחה.
...
ברירת המחדל היא 100 samples למחלקה. זה מספר פתיחה, לא ערובה לאיכות. 400
פריימים מאדם אחד ברצף אחד הם פחות טובים מ־400 פריימים שחולקו בין כמה אנשים,
...
1. invalid ו־0 הופכים לערך רקע 0;
2. מתוך pixels בטווח 100..1200mm מחושב percentile 10 של האזור הקרוב;
3. נשמרת רצועת עומק עד 250mm מאחוריו—היד/האובייקט הקרוב—והשאר הופך רקע;
4. מתבצע crop עם margin של 2 pixels סביב ה־foreground;
5. ה־crop מוגדל ב־nearest-neighbor תוך שמירת יחס ממדים וממורכז ב־64×50;
6. קרוב הופך 255, רחוק/רקע 0, ומתקבל `uint8` NHWC `[N,50,64,1]`.
...
`05_TRAIN.bat` בונה `spatial_cnn_v2`: ארבע שכבות Conv2D בשלושה scales,
שלוש MaxPool, GlobalAveragePooling ו־Dense קטן לפני softmax. receptive field
עמוק יותר שומר תבנית של אצבעות/כף יד גם כשהיד זזה בין bursts. הרשת עדיין
פשוטה בכוונה:
...
3. לפחות 3–5 sessions של `01_CAPTURE.bat`, כמה bursts לכל מחלקה.
